...
- **Cost budget**: evaluation cost is tracked as first-class; a hard circuit breaker stops spend before the cap is exceeded, not just after, and every run reports actual spend.
...
- **Commoditized, so present but not the pitch:** tool-call correctness, efficiency metrics, and holistic LLM-as-judge. eval-quality supports them; it does not pretend they are the differentiator.
...
**What actually makes it different** is the *combination*, not any one metric: (1) **TypeScript-native and framework-agnostic** - the real gap, since the serious TS options are platform- or runtime-bound; and (2) the **trajectory-as-evidence framing** wired to a ship/don't-ship gate, leading with evidence-grounded faithfulness and path-invariant scoring rather than a metrics menu. Execution, language niche, and framing are the edge - stated plainly. The differentiators are well-supported by current research; the commodity metrics are not claimed as novel.
...
- The BMAD Test Architect "AE" workflow - the methodology layer that *consumes* this engine (out of scope here).
- Aggregating agent-eval alongside unit/contract/E2E/perf/observability under one gate - a longer-term direction, not a v0 deliverable.
- Additional provider adapters beyond Anthropic (interface exists; more adapters come later).
- Hosted service, dashboards, or UI.

## Vision

If it succeeds, eval-quality becomes a common way TypeScript teams produce **agent-behavior evidence** for a release gate - the missing check next to unit, contract, E2E, perf, and observability. The longer-term direction is simple: every quality signal, deterministic and non-deterministic alike, feeding one ship/don't-ship decision. The open Test Architect methodology (the "AE" workflow, in BMAD) can build on top of it to encode *how to evaluate agents correctly*. The engine is open and provider-agnostic on purpose - it's a community tool, not a funnel.
...
2. **BMAD Test Architect "AE" workflow - the methodology layer** that *consumes* this engine and encodes *how to evaluate agents correctly*. Out of scope for this PRD; noted so the engine's interfaces stay clean enough to be wrapped later.

Design implication for v0: keep the engine's outputs (evidence artifacts) and grader interfaces stable and composable, because a methodology layer will orchestrate them.
...
- v0 scope tight and shippable; cross-signal aggregation = direction, not build.
